...
def _generate_fic_plots(results: dict):
    ...
    for loop_num in (100, 101, 102):
        ...
        fig, _ = plot_step_response_multiple(
            ...
            R_py,
            step_time=FIC_STEP_TIME,
            step_info_dict={"Python": results[f"si_fic{loop_num}_py"], "HYSYS": results[f"si_fic{loop_num}_hy"]},
            y_initial=FIC_SP_INITIAL,
            settling_threshold=THRESH_FIC,
...
def _generate_lic_plots(results: dict):
    ...
    for label in ("Tight", "Averaging"):
        ...
        fig, _ = plot_step_response_multiple(
            ...
            step_time=LIC_SP_STEP_TIME,
            step_info_dict={"Python": results["lic_sp_si"][label], "HYSYS": results["lic_sp_hy_si"][label]},
            y_initial=LIC_SP_INITIAL,
            settling_threshold=THRESH_LIC,
            title=f"LIC-100 ({label}): Python vs HYSYS — Setpoint Tracking",
            ylabel="Transmitter Output (%TO)",
            curve_colors={"Python": "#0055cc", "HYSYS": "#d65a00"},
            savefig_path=_out("LIC-100", "Comparison", f"LIC100_{label}_Python_vs_HYSYS_Setpoint.png"),
            dpi=DPI,
        )
        plt.close(fig)
    ...
    for label in ("Tight", "Averaging"):
        df_py = _load(os.path.join(DATA_DIR, "LIC-100", "Python", f"LIC_100_{label}_Disturbance_Scenario.CSV"))
        df_hy = _load(os.path.join(DATA_DIR, "LIC-100", "HYSYS", f"LIC_100_{label}_disturbance_scenario.CSV"))
        t_ref = _arr(df_py, "Time")
        C_hy = _interp(t_ref, df_hy, "LIC-100 - PV")
        ...
        fig, _ = plot_simple_response_multiple(
            ...
            ylabel="Transmitter Output (%TO)",
            curve_colors={"Python": "#0055cc", "HYSYS": "#d65a00"},
            savefig_path=_out("LIC-100", "Comparison", f"LIC100_{label}_Python_vs_HYSYS_Disturbance.png"),
            dpi=DPI,
        )
        plt.close(fig)

    # Tight vs Averaging Disturbance comparison (Python)
    df_td = _load(os.path.join(DATA_DIR, "LIC-100", "Python", "LIC_100_Tight_Disturbance_Scenario.CSV"))
    df_ad = _load(os.path.join(DATA_DIR, "LIC-100", "Python", "LIC_100_Averaging_Disturbance_Scenario.CSV"))
    ...
    df_py_t_dist = _load(os.path.join(DATA_DIR, "LIC-100", "Python", "LIC_100_Tight_Disturbance_Scenario.CSV"))
    t_t_dist = _arr(df_py_t_dist, "Time")
    df_py_a_sp = _load(os.path.join(DATA_DIR, "LIC-100", "Python", "LIC_100_Averaging_Setpoint.CSV"))
    df_py_a_dist = _load(os.path.join(DATA_DIR, "LIC-100", "Python", "LIC_100_Averaging_Disturbance_Scenario.CSV"))
    ...
    fig_comp_sp_pv, _ = plot_step_response_multiple(
        t_t_sp,
        {"Tight": _arr(df_py_t_sp, "LT-100 - C"), "Averaging": _interp(t_t_sp, df_py_a_sp, "LT-100 - C")},
    ...
    fig_comp_sp_co, _ = plot_simple_response_multiple(
        t_t_sp,
        {"Tight": _arr(df_py_t_sp, "LC-100 - M"), "Averaging": _interp(t_t_sp, df_py_a_sp, "LC-100 - M")},
    ...
    fig_comp_dist_pv, _ = plot_simple_response_multiple(
        t_t_dist,
        {"Tight": _arr(df_py_t_dist, "LT-100 - C"), "Averaging": _interp(t_t_dist, df_py_a_dist, "LT-100 - C")},
    ...
    fig_comp_dist_co, _ = plot_simple_response_multiple(
        t_t_dist,
        {"Tight": _arr(df_py_t_dist, "LC-100 - M"), "Averaging": _interp(t_t_dist, df_py_a_dist, "LC-100 - M")},
    ...
    for label in ("Tight", "Averaging"):
        ...
        df_py_sp = _load(os.path.join(DATA_DIR, "LIC-100", "Python", f"LIC_100_{label}_Setpoint.CSV"))
        df_hy_sp = _load(os.path.join(DATA_DIR, "LIC-100", "HYSYS", f"LIC_100_{label}_setpoint.CSV"))
        ...
        df_py_dist = _load(os.path.join(DATA_DIR, "LIC-100", "Python", f"LIC_100_{label}_Disturbance_Scenario.CSV"))
        df_hy_dist = _load(os.path.join(DATA_DIR, "LIC-100", "HYSYS", f"LIC_100_{label}_disturbance_scenario.CSV"))
...
def _generate_tic_plots(results: dict):
    ...
    t_ref = _arr(_load(os.path.join(DATA_DIR, "TIC-100", "Python", "setpoint-tuning", "TIC_100_QDR.CSV")), "Time")
    M_dict_sp = {
        k: _arr(_load(os.path.join(DATA_DIR, "TIC-100", "Python", "setpoint-tuning", f"TIC_100_{k}.CSV")), "TC-100 - M")
    ...
    t_dist_ref = _arr(_load(os.path.join(DATA_DIR, "TIC-100", "Python", "disturbance-tuning", "TIC_100_QDR.CSV")), "Time")
    M_dict_dist = {
        k: _arr(_load(os.path.join(DATA_DIR, "TIC-100", "Python", "disturbance-tuning", f"TIC_100_{k}.CSV")), "TC-100 - M")
        for k in ("Syn", "QDR", "IAE")
    }
    fig, _ = plot_simple_response_multiple(
        t_dist_ref,
    ...
    df_ref = _load(os.path.join(DATA_DIR, "TIC-100", "Python", "setpoint-tuning", "TIC_100_QDR.CSV"))
    t_dual = _arr(df_ref, "Time")
    u_dual = _arr(df_ref, "TSP-100 - R")
    y_dual = {
        k: _arr(_load(os.path.join(DATA_DIR, "TIC-100", "Python", "setpoint-tuning", f"TIC_100_{k}.CSV")), "TT-100 - C")
        for k in ("Syn", "QDR", "IAE")
    }
    fig, _ = plot_dual_response(
        t_dual,
    ...
    df_py_q = _load(os.path.join(DATA_DIR, "TIC-100", "Python", "setpoint-tuning", "TIC_100_QDR.CSV"))
    df_hy_q = _load(os.path.join(DATA_DIR, "TIC-100", "HYSYS", "setpoint-tuning", "TIC_100_QDR.CSV"))
    ...
    fig, _ = plot_dual_response(
        ...
        step1_end_time=TIC_DIST_END_TIME,
        step2_time=TIC_SP_STEP_TIME,
        step_info_dict={"Python": results["si_nested"]["QDR"], "HYSYS": results["si_nested_hy"]["QDR"]},
        step1_threshold=THRESH_TIC_DIST,
        step2_threshold=THRESH_TIC,
    ...
    for tuning in ("Syn", "IAE"):
        df_py_t = _load(os.path.join(DATA_DIR, "TIC-100", "Python", "setpoint-tuning", f"TIC_100_{tuning}.CSV"))
        df_hy_t = _load(os.path.join(DATA_DIR, "TIC-100", "HYSYS", "setpoint-tuning", f"TIC_100_{tuning}.CSV"))
        ...
        fig, _ = plot_dual_response(
            ...
            step1_end_time=TIC_DIST_END_TIME,
            step2_time=TIC_SP_STEP_TIME,
            step_info_dict={"Python": results["si_nested"][tuning], "HYSYS": results["si_nested_hy"][tuning]},
            step1_threshold=THRESH_TIC_DIST,
            step2_threshold=THRESH_TIC,
    ...
    for tuning in ("Syn", "QDR", "IAE"):
        df_py_d = _load(os.path.join(DATA_DIR, "TIC-100", "Python", "disturbance-tuning", f"TIC_100_{tuning}.CSV"))
        df_hy_d = _load(os.path.join(DATA_DIR, "TIC-100", "HYSYS", "disturbance-tuning", f"TIC_100_{tuning}.CSV"))
        ...
        fig, _ = plot_dual_response(
            ...
            ylabel="Transmitter Output (%TO)",
            curve_colors={"Python": "#0055cc", "HYSYS": "#d65a00"},
            savefig_path=_out("TIC-100", "Comparison", f"TIC100_{tuning}_Python_vs_HYSYS_Disturbance.png"),
            dpi=DPI,
        )
        plt.close(fig)
    ...
    df_ref_d = _load(os.path.join(DATA_DIR, "TIC-100", "Python", "disturbance-tuning", "TIC_100_QDR.CSV"))
    t_dual_d = _arr(df_ref_d, "Time")
    u_dual_d = _arr(df_ref_d, "TSP-100 - R")
    y_dual_d = {
        k: _arr(_load(os.path.join(DATA_DIR, "TIC-100", "Python", "disturbance-tuning", f"TIC_100_{k}.CSV")), "TT-100 - C")
        for k in ("Syn", "QDR", "IAE")
    }
    si_nested_dist_py = {
        k: {
    ...
    df_py = _load(os.path.join(DATA_DIR, "TIC-100", "Python", "disturbance-tuning", "TIC_100_QDR.CSV"))
    df_hy = _load(os.path.join(DATA_DIR, "TIC-100", "HYSYS", "disturbance-tuning", "TIC_100_QDR.CSV"))
    t_py = _arr(df_py, "Time")
    ...
    op_hy = 100.0 - _interp(t_py, df_hy, "TIC-100 - OP")  # Invert HYSYS to match Python's 80% convention
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.09), ncol=2, fontsize=10, frameon=True, framealpha=0.95, edgecolor='gray', fancybox=True)
    ...
    _save_fig(fig_co_comp, _out("TIC-100", "report", "TIC100_CO_Comparison_Python_vs_HYSYS.png"), DPI)
    plt.close(fig_co_comp)
