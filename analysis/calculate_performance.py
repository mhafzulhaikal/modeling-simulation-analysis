...
DATA_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'outputs', 'data', 'step_test_data'
)
...
VALIDATION_CASES = [
    ...
    },
    {
        ...
        'py_path': os.path.join(
            DATA_DIR, 'LIC-100', 'Python', 'LIC_100_Tight_Disturbance_Scenario.CSV'
        ),
        'hy_path': os.path.join(
            DATA_DIR, 'LIC-100', 'HYSYS', 'LIC_100_Tight_disturbance_scenario.CSV'
        ),
    },
    {
        'loop': 'LIC-100',
        ...
        'py_path': os.path.join(
            DATA_DIR, 'LIC-100', 'Python', 'LIC_100_Averaging_Disturbance_Scenario.CSV'
        ),
        'hy_path': os.path.join(
            DATA_DIR, 'LIC-100', 'HYSYS', 'LIC_100_Averaging_disturbance_scenario.CSV'
        ),
    },
    {
        'loop': 'TIC-100',
        ...
        'py_path': os.path.join(
            DATA_DIR, 'TIC-100', 'Python', 'setpoint-tuning', 'TIC_100_Syn.CSV'
        ),
    ...
    },
    {
        ...
        'py_path': os.path.join(
            DATA_DIR, 'TIC-100', 'Python', 'setpoint-tuning', 'TIC_100_QDR.CSV'
        ),
    ...
    },
    {
        ...
        'py_path': os.path.join(
            DATA_DIR, 'TIC-100', 'Python', 'setpoint-tuning', 'TIC_100_IAE.CSV'
        ),
    ...
    },
    {
        ...
        'py_path': os.path.join(
            DATA_DIR, 'TIC-100', 'Python', 'disturbance-tuning', 'TIC_100_Syn.CSV'
        ),
        'hy_path': os.path.join(
            DATA_DIR, 'TIC-100', 'HYSYS', 'disturbance-tuning', 'TIC_100_Syn.CSV'
        ),
    },
    {
        'loop': 'TIC-100',
        ...
        'py_path': os.path.join(
            DATA_DIR, 'TIC-100', 'Python', 'disturbance-tuning', 'TIC_100_QDR.CSV'
        ),
        'hy_path': os.path.join(
            DATA_DIR, 'TIC-100', 'HYSYS', 'disturbance-tuning', 'TIC_100_QDR.CSV'
        ),
    },
    {
        'loop': 'TIC-100',
        ...
        'py_path': os.path.join(
            DATA_DIR, 'TIC-100', 'Python', 'disturbance-tuning', 'TIC_100_IAE.CSV'
        ),
        'hy_path': os.path.join(
            DATA_DIR, 'TIC-100', 'HYSYS', 'disturbance-tuning', 'TIC_100_IAE.CSV'
        ),
    },
]
...
def align_time_series(
    df_python: pd.DataFrame, df_hysys: pd.DataFrame, time_col: str
...
def calculate_validation_metrics(
    y_hysys: np.ndarray, y_python: np.ndarray, epsilon: float = 1e-8
) -> dict[str, Any]:
    ...
    if n_total == 0:
        return {
            ...
            'PBIAS': float('nan'),
            'R2': float('nan'),
            'Warnings': ['No valid overlapping data samples.'],
        }
...
def write_summary(validation_results: list[dict[str, Any]], txt_path: str) -> None:
    ...
    lines.append(W_SEP)
    lines.append(
        '  This section provides a guide for interpreting the validation metrics and plots:'
    )
    lines.append('')
    lines.append('  1. MEAN ABSOLUTE PERCENTAGE ERROR (MAPE):')
    lines.append(
        '     - MAPE is useful as a relative error metric, but it should not be used alone.'
    )
    ...
    lines.append(
        '       quadratically, making it more sensitive to transient spikes and large lags.'
    )
    lines.append('')

    lines.append('  4. MEAN BIAS ERROR (MBE) & PERCENT BIAS (PBIAS):')
    lines.append(
        '     - Bias or PBIAS indicates whether Python systematically overpredicts or underpredicts HYSYS.'
    )
    lines.append(
        '       Positive bias shows overprediction; negative bias indicates underprediction.'
    )
    lines.append('')
...
def _write_summary(registry: dict, txt_path: str, csv_path: str) -> None:
    ...
    def _fv(v, fmt):
        return NA if (np.isnan(float(v)) or np.isinf(float(v))) else format(float(v), fmt)
    ...
    for (loop, source, scenario), si in registry.items():
        ...
        iae = _fv(d['IAE'], '.2f')
        lines.append(
            f'  {loop:<7}  {source:<6}  {scenario:<27}  {rt:>9}  {ts:>9}  {os_:>8}  {iae:>10}'
        )
    ...
    if matched_keys:
        ...
        overall_errors = {k: {'abs_pct': [], 'abs': [], 'sq': []} for k in metrics_labels}
        ...
        for key in matched_keys:
            ...
            scenario_valid_pcts = [
                raw_devs[key][ml] for ml in valid_metrics if not np.isnan(raw_devs[key][ml])
            ]
        ...
        for loop_group in loop_group_order:
            ...
            valid_metric_mapes = [
                loop_group_mapes[loop_group][ml]
                for ml in group_valid_metrics.get(loop_group, [])
                if not np.isnan(loop_group_mapes[loop_group][ml])
            ]
            ...
            lines.append(f'  {loop_group:<7}  {str(n_pairs) + " pairs":<27}' + ''.join(parts))
        ...
        for ml in metrics_labels:
            ...
            group_vals = [
                loop_group_mapes[lg][ml]
                for lg in loop_group_order
                if ml in group_valid_metrics.get(lg, []) and not np.isnan(loop_group_mapes[lg][ml])
            ]
            if group_vals:
        ...
        valid_group_mapes = [
            loop_group_single_mape[lg]
            for lg in loop_group_order
            if not np.isnan(loop_group_single_mape[lg])
        ]
        ...
        lines.append(f'  {"OVERALL":<7}  {str(len(matched_keys)) + " pairs":<27}' + ''.join(parts))
        lines.append(dev_sep)
    ...
    for (loop, source, scenario), si in registry.items():
        ...
        row['PerformanceIndex'] = nm.get('PerformanceIndex', float('nan')) if nm else float('nan')
        rows.append(row)
...
def calculate_all_performance() -> dict[str, Any]:
    ...
    for tuning in ('Syn', 'QDR', 'IAE'):
        ...
        si_dist = _si(
            ...
            y0=50.0,
            yf=50.0,
            step_time=TIC_DIST_STEP_TIME,
            end_time=TIC_DIST_END_TIME,
            thresh=THRESH_TIC_DIST,
        )
    ...
    for tuning in ('Syn', 'QDR', 'IAE'):
        ...
        tic_dist_py_sp_si[tuning] = _si(
            t, C, R, TIC_SP_INITIAL, TIC_SP_FINAL, TIC_SP_STEP_TIME, thresh=THRESH_TIC
        )
    ...
    for tuning in ('Syn', 'QDR', 'IAE'):
        ...
        tic_dist_hy_sp_si[tuning] = _si(
            t_d, C_d, R_d, TIC_SP_INITIAL, TIC_SP_FINAL, TIC_SP_STEP_TIME, thresh=THRESH_TIC
        )
...
def main() -> None:
    ...
    for r in validation_results:
        ...
        json_data[key] = {
            k: v for k, v in r.items() if k not in ('loop', 'scenario', 'variable', 'Warnings')
        }
    ...
    integral_logs = [
        '',
        '=' * 80,
        '  CONTROL LOOP INTEGRAL PERFORMANCE ERRORS (SP vs PV)',
        '=' * 80,
    ]
    ...
    for case in VALIDATION_CASES:
        ...
        # Python SP vs PV
        err_py = np.asarray(df_py_aligned[py_sp_col].values, dtype=float) - np.asarray(
            df_py_aligned[py_pv_col].values, dtype=float
        )
        int_py = calculate_integral_error(t_ref, err_py)

        # HYSYS SP vs PV
        err_hy = np.asarray(df_hy_aligned[hy_sp_col].values, dtype=float) - np.asarray(
            df_hy_aligned[hy_pv_col].values, dtype=float
        )
        int_hy = calculate_integral_error(t_ref, err_hy)

        integral_logs.append(f'\n  [ {loop} - {scenario} ]')
        integral_logs.append(
            f'    Python: IAE = {int_py["IAE"]:12.2f} | ISE = {int_py["ISE"]:12.2f} | ITAE = {int_py["ITAE"]:12.2f}'
        )
        integral_logs.append(
            f'    HYSYS : IAE = {int_hy["IAE"]:12.2f} | ISE = {int_hy["ISE"]:12.2f} | ITAE = {int_hy["ITAE"]:12.2f}'
        )
